...
export class ArticlesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.rightSideAtriclesSub = this.articleDataStorageService
      ...
      .subscribe(() => {
        this.rightSideAtricles = ArticleUtils.convertStringToJson(
          this.articlesService.getArticles(),
          'Items'
        );
        this.rightSideArticlesLazyLoad = ArticleUtils.contentLazyLoad(
          this.rightSideAtricles
        ).articlesLazyLoad;
      });
    ...
    this.leftSideArticlesSub = this.articleDataStorageService
      ...
      .subscribe(
        () =>
          (this.leftSideArticles = ArticleUtils.convertStringToJson(
            this.articlesService.getArticles(),
            'Items'
          ))
      );
    ...
    this.contentArticlesObsSub = this.articlesLeftSideService.articlesObs
      .subscribe(
        (articles) => {
          this.contentTitleLazyLoad = ArticleUtils.contentLazyLoad(
            articles
          ).articleCategoryTitlesLazyLoad;
          this.contentArticleLazyLoad = ArticleUtils.contentLazyLoad(
            articles
          ).articlesLazyLoad;
        }
      );
  ...
  onSidesToggle(side: string) {
    if (side === 'left') this.isLeftSideMenuOpen = !this.isLeftSideMenuOpen;
    else this.isRightSideMenuOpen = this.isRightSideMenuOpen;
  }

  onScrollRightSide(event: Event) {
    const { allArticles } = ArticleUtils.getAllArticleCategoryTitlesAndItems(
      this.rightSideAtricles
    );
    const articles = ArticleUtils.getArticleItemsForLazyLoading(
      allArticles,
      this.rightSideLazyLoadPageNumber++
    );

    this.rightSideArticlesLazyLoad = this.rightSideArticlesLazyLoad.concat(
      articles
    );
  }
  ...
  onScrollContents(event: Event) {
    ...
    const articles = ArticleUtils.getArticleItemsForLazyLoading(
      allArticles,
      this.contentLazyLoadPageNumber++
    );
  ...
  setLatestArticlesFromLeftSide() {
    this.isLatestArticlesLinkObsSub = this.articlesLeftSideService.isLatestArticlesLinkObs.subscribe(
      () => this.getLatestArticles()
    );
  }
  ...
  private getLatestArticles() {
    this.contentArticlesSub = this.articleDataStorageService
      ...
      .subscribe(() => {
        ...
        this.contentArticles = ArticleUtils.convertStringToJson(
          articlesObjectStr,
          'Items'
        );
